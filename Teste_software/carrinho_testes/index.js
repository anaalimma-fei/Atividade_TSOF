...
try{
    ...
    console.log(`[CT-03] Eps: Erro | Obtido: ${res3} -> FALHOU (Não gerou erro)`);
} catch (e) {
    console.log(`[Ct-03] Esp: Erro | Obtido: Erro (${e.message}) -> PASSOU`);
}

//CT-04: Arredondamemto de Centavos
try{
  const res4 = calcularTotal([{ preco: 33.333, quantidade: 1}], null);
  console.log(`[CT-04] Eps: 48.33| Obtido: ${res4} -> ${res4 === 48.33 ? 'PASSOU' : 'FALHOU'}`);
} catch (e) {
...
try{
  ...
  console.log(`[CT-06] Eps: 95| Obtido: ${res6} -> ${res6 === 95 ? 'PASSOU' : 'FALHOU'}`);
} catch (e) {
